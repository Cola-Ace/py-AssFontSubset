    ...
    for line in lines:
        if line.startswith("Style:"):
            ...
            if len(style_data) > 1:
                ...
                styles[style_name] = style_data[1].strip()
            continue
...
def load_all_fonts():
    ...
    try:
        ...
        for fonts_dir in font_dirs:
            font_files = [os.path.join(fonts_dir, file) for file in os.listdir(fonts_dir) if file.endswith(('.ttf', '.otf', '.ttc'))]
...
def modify_subtitle_file(subtitle_file, font_mapping, output_dir):
    try:
        ...
        font_subset_info.append(f'; Generated by Subsetter v0.1.0 at {current_time}\n')
